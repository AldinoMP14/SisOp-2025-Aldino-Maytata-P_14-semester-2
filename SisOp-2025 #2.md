...
In an SMP (Symmetric Multiprocessing) system, multiple processors have their own local caches, which can lead to cache coherence issues.
Example:
1.	Processor 1 modifies variable X stored in its local cache.
2.	Processor 2 still has the old value of X in its cache.
3.	This creates an inconsistency between the caches.
Solution:
1.	MESI Protocol (Modified, Exclusive, Shared, Invalid): Ensures only one processor can modify a cache block at a time.
2.	Write-through cache: Updates main memory immediately to maintain consistency.
---
